...
# Deployed execution script for graph unload_table compiled at 2017-03-13 05:42:31 using Co>Operating System version 3.2.6.4
# 
...
it.  Please define the environment variable DB_SXCA6_DIR to be the project
...
# Project parameters:
export DB_SXCA6_DIR;DB_SXCA6_DIR=${DB_SXCA6_DIR:-"$(cd ${__ab_script_dir}/..; pwd)"}
case "$_ab_uname" in
CYGWIN_* )
   DB_SXCA6_DIR="$(cygpath -m "$DB_SXCA6_DIR")"
esac
typeset _AB_SAVED_PROJECT_DIR
_AB_SAVED_PROJECT_DIR="${DB_SXCA6_DIR}"
export AB_GRAPH_NAME;AB_GRAPH_NAME=unload_table

# Host Setup Commands:
. /etl/etc/environment
_AB_PROXY_DIR="$(pwd)/"unload_table-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${DB_SXCA6_DIR}"'/mp/unload_table.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${DB_SXCA6_DIR}"'/mp/unload_table.mp'   -prohibit-legacy-codegen
   mpjret=$?
