...
if [ X"${1}" != X"" ]; then
   export DB_CCTA_DIR;   DB_CCTA_DIR="${1}"
   shift
fi
if [ X"${DB_CCTA_DIR:-}" = X"" ]; then
   print -r -- 'DB_CCTA_DIR is undefined.  Pass its value as the first argument to this script'
   export __AB_ERROR;   __AB_ERROR=1
fi
case $_ab_uname in
CYGWIN_* )
   DB_CCTA_DIR="${DB_CCTA_DIR:+$(cygpath -m "$DB_CCTA_DIR")}"
esac
if [ X"${__AB_ERROR}" != X1 ]; then
   _AB_CALLING_PROJECT=""
   if [ $# -gt 0 ]; then
      __AB_INVOKE_PROJECT "${DB_CCTA_DIR}"/.project.ksh "${DB_CCTA_DIR}" define start "$@"
   else
      __AB_INVOKE_PROJECT "${DB_CCTA_DIR}"/.project.ksh "${DB_CCTA_DIR}" define start
   fi
fi
...
function unset_project_env {
   _AB_CALLING_PROJECT=""
   if [ $# -gt 0 ]; then
      __AB_INVOKE_PROJECT "${DB_CCTA_DIR}"/.project.ksh "${DB_CCTA_DIR}" undefine start "$@"
   else
      __AB_INVOKE_PROJECT "${DB_CCTA_DIR}"/.project.ksh "${DB_CCTA_DIR}" undefine start
   fi
};
