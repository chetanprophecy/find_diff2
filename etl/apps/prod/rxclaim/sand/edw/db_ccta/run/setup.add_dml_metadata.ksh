...
export AB_HOME;AB_HOME=${AB_HOME:-/u01/abinitio/abinitio-V3-1-7}
...
export AB_COMPATIBILITY;AB_COMPATIBILITY=3.1.7.7

# Deployed execution script for graph "setup.add_dml_metadata", compiled at Thursday, September 11, 2014 22:42:52 using GDE version 3.1.7.5
...
it.  Please define the environment variable DB_CCTA_DIR to be the project
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=setup_add_dml_metadata

_AB_PROXY_DIR="$(pwd)"/setup.add_dml_metadata-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
# Project Parameters:
export DB_CCTA_DIR;DB_CCTA_DIR=${DB_CCTA_DIR:-"$(cd ${__ab_script_dir}/..; pwd)"}
case "$_ab_uname" in
CYGWIN_* )
   DB_CCTA_DIR="$(cygpath -m "$DB_CCTA_DIR")"
esac
typeset _AB_SAVED_PROJECT_DIR
_AB_SAVED_PROJECT_DIR="${DB_CCTA_DIR}"

if [ $# -gt 0 ]; then
   air sandbox run "${DB_CCTA_DIR}"'/mp/setup.add_dml_metadata.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${DB_CCTA_DIR}"'/mp/setup.add_dml_metadata.mp'   -prohibit-legacy-codegen
   mpjret=$?
