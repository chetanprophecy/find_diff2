...
if [ X"${1}" != X"" ]; then
   export DB_SXCA5_DIR;   DB_SXCA5_DIR="${1}"
   shift
fi
if [ X"${DB_SXCA5_DIR:-}" = X"" ]; then
   print -r -- 'DB_SXCA5_DIR is undefined.  Pass its value as the first argument to this script'
   export __AB_ERROR;   __AB_ERROR=1
fi
case $_ab_uname in
CYGWIN_* )
   DB_SXCA5_DIR="${DB_SXCA5_DIR:+$(cygpath -m "$DB_SXCA5_DIR")}"
esac
if [ X"${__AB_ERROR}" != X1 ]; then
   _AB_CALLING_PROJECT=""
   if [ $# -gt 0 ]; then
      __AB_INVOKE_PROJECT "${DB_SXCA5_DIR}"/.project.ksh "${DB_SXCA5_DIR}" define start "$@"
   else
      __AB_INVOKE_PROJECT "${DB_SXCA5_DIR}"/.project.ksh "${DB_SXCA5_DIR}" define start
   fi
fi
...
function unset_project_env {
   _AB_CALLING_PROJECT=""
   if [ $# -gt 0 ]; then
      __AB_INVOKE_PROJECT "${DB_SXCA5_DIR}"/.project.ksh "${DB_SXCA5_DIR}" undefine start "$@"
   else
      __AB_INVOKE_PROJECT "${DB_SXCA5_DIR}"/.project.ksh "${DB_SXCA5_DIR}" undefine start
   fi
};
