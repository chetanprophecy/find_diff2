...
#   ksh -c "cd ..; . ./ab_project_setup.ksh .; $DB_SXCA5_BIN/analyze_dependencies.ksh"

if [[ -z "$DB_SXCA5_DML" ]]; then
  echo 'Error: Sandbox Parameters Missing.  Try setting them with "cd Project_Dir; . ./ab_project_setup.ksh .".' >&2
  echo "DB_SXCA5_DML=$DB_SXCA5_DML" >&2
  exit 1
fi

# Remove any left over datasets.
for t in $(air ls "$AI_RPATH/edw/db_sxca5/tables/$DB_SXCA5_DB_NAME_REPO" | grep dataset: | awk '{print $NF}'); do
  p="$(echo $t | sed -e 's/_sta*g[eing]*[.]/./' -e 's/_[iku]$//')" # Convert the table name into a pset name
  db="$(echo $p | cut -d . -f 1)"
  if [[ ! -f "$DB_SXCA5_DML/../pset/setup.datasets.$db/setup.datasets.$p.pset" ]]; then
    echo air rm "$AI_RPATH/edw/db_sxca5/tables/$DB_SXCA5_DB_NAME_REPO/$t"
    air      rm "$AI_RPATH/edw/db_sxca5/tables/$DB_SXCA5_DB_NAME_REPO/$t"
  fi
done

# Update all related datasets.
air project analyze-dependencies "$AI_RPATH/edw/db_sxca5" -all
