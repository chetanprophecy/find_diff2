...
select 'D_MEMBER_ELIGIBILITY', max(update_ts),count_big(*) from orx_idw_dm_prd.D_MEMBER_ELIGIBILITY  where MEL_THRU_DT >= '2020-01-01' AND MEL_STAT_CD = 'A' union
select 'D_MEMBER_ELIGIBILITY_ADJUD', max(update_ts),count_big(*) from orx_idw_dm_prd.D_MEMBER_ELIGIBILITY_ADJUD union
select 'D_MMD_MEMBER_ELIGIBILITY', max(update_ts),count_big(*) from orx_idw_dm_prd.D_MMD_MEMBER_ELIGIBILITY where MMD_THRU_DT  >= '2020-01-01'  union
select 'F_CLAIM_TRANSACTION', max(update_ts),count_big(*) from orx_idw_dm_prd.F_CLAIM_TRANSACTION where SBM_DT_SK >= 20200101 union
select 'F_CLAIM_MIC_DETAIL', max(update_ts),count_big(*) from orx_idw_dm_prd.F_CLAIM_MIC_DETAIL   where SBM_DT_SK >= 20200101 union
select 'F_CLAIM_TRANSACTION_EXTN', max(update_ts),count_big(*) from orx_idw_dm_prd.F_CLAIM_TRANSACTION_EXTN where SBM_DT_SK >= 20200101 union
...
select 'F_CPN_ADJ_CLAIM_TRANSACTION', max(update_ts),count_big(*) from orx_idw_dm_prd.F_CPN_ADJ_CLAIM_TRANSACTION where SBM_DT_SK >= 20200101 union
select 'F_FORMULARY_CLAIM_TRX', max(update_ts),count_big(*) from orx_idw_dm_prd.F_FORMULARY_CLAIM_TRX where SBM_DT_SK >= 20200101 union
select 'F_MPD_CLAIM_TRANSACTION', max(update_ts),count_big(*) from orx_idw_dm_prd.F_MPD_CLAIM_TRANSACTION where SBM_DT_SK >= 20200101 union
select 'F_PAYMENT_CLAIM_TXN', max(update_ts),count_big(*) from orx_idw_dm_prd.F_PAYMENT_CLAIM_TXN where SBM_DT_SK >= 20200101 union
select 'F_SAVING_ACCT_PAY_DTL', max(update_ts),count_big(*) from orx_idw_dm_prd.F_SAVING_ACCT_PAY_DTL  union
select 'F_CDUR_EVENTS', max(update_ts),count_big(*) from orx_idw_dm_prd.F_CDUR_EVENTS where SBM_DT >= '2020-01-01' union
select 'F_WC_CLAIM_TRANSACTION', max(update_ts),count_big(*) from orx_idw_dm_prd.F_WC_CLAIM_TRANSACTION where SBM_DT_SK >= 20200101 union
