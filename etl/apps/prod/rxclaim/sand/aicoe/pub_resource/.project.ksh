...
if [ X"${_AB_recursion_aa579084_e96ddcf4e92_71ac:-}" = X"" ]; then
   _AB_recursion_aa579084_e96ddcf4e92_71ac=1
else
...
function _ab_define_parameters {
   ...
   typeset _ab_tmp_scriptid=aa579084_e96ddcf4eaa_71ac_1
   typeset _ab_tmp_file=_ab_tmp_${_ab_tmp_scriptid}_$$.ksh
   if [ X"${_ab_running_from_gde}" = "X0"  ]; then
      _AB_tmp_parameters_aa579084_e96ddcf4eaa_71ac_1="${_ab_tmp_dir}"/"${_ab_tmp_file}"
      trap 'rm -f "${_AB_tmp_parameters_aa579084_e96ddcf4eaa_71ac_1}" ' EXIT
      # Work around pdksh bug: the EXIT handler is not executed upon a signal.
      trap '_AB_status=$?; rm -f "${_AB_tmp_parameters_aa579084_e96ddcf4eaa_71ac_1}"; exit $_AB_status' HUP INT QUIT TERM
   else
      # Temp files will be in _AB_PROXY_DIR folder
      # _AB_PROXY_DIR folder will be deleted by the top level script
      _AB_tmp_parameters_aa579084_e96ddcf4eaa_71ac_1="${_AB_PROXY_DIR}"/"${_ab_tmp_file}"
   fi
   ...
   rm -f "${_AB_tmp_parameters_aa579084_e96ddcf4eaa_71ac_1}"
   
   if [ $# -gt 0 ]; then
      "${AIRPROGRAM}"  sandbox scripts -inner  "${_AB_PROJECT_DIR}"  "${_AB_DEFINE_OR_EXECUTE}" "${_AB_START_OR_END}"   "$@" >  "${_AB_tmp_parameters_aa579084_e96ddcf4eaa_71ac_1}"
   else
      "${AIRPROGRAM}"  sandbox scripts -inner  "${_AB_PROJECT_DIR}"  "${_AB_DEFINE_OR_EXECUTE}" "${_AB_START_OR_END}"   >  "${_AB_tmp_parameters_aa579084_e96ddcf4eaa_71ac_1}"
   fi
   ...
   .  "${_AB_tmp_parameters_aa579084_e96ddcf4eaa_71ac_1}"
   rm -f "${_AB_tmp_parameters_aa579084_e96ddcf4eaa_71ac_1}"
   if [ X"${_ab_running_from_gde}" = "X0"  ]; then
...
unset _AB_recursion_aa579084_e96ddcf4e92_71ac
