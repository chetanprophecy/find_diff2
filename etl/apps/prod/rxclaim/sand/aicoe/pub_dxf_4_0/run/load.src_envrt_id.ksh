...
# Deployed execution script for graph load.src_envrt_id compiled at 2016-10-25 15:36:20 using Co>Operating System version 3.2.6.4
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=load.src_envrt_id

# Host Setup Commands:
. /etl/etc/environment
_AB_PROXY_DIR="$(pwd)/"load.src_envrt_id-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_DXF_DIR}"'/mp/load.src_envrt_id.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_DXF_DIR}"'/mp/load.src_envrt_id.mp'   -prohibit-legacy-codegen
   mpjret=$?
