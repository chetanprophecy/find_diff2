...
# Deployed execution script for graph get.db compiled at 2016-11-02 08:15:38 using Co>Operating System version 3.2.6.4
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.rebates/main/users/ndagade/aicoe/pub_dxf}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=get.db

# Host Setup Commands:
. /etl/etc/environment
_AB_PROXY_DIR="$(pwd)/"get.db-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/get.db.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/get.db.mp'   -prohibit-legacy-codegen
   mpjret=$?
