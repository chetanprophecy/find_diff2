...
# Deployed execution script for graph Universal%20Key%20Extraction compiled at 2016-04-27 01:19:29 using Co>Operating System version 3.2.4.1
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME='Universal%20Key%20Extraction'

_AB_PROXY_DIR="$(pwd)/"Universal_Key_Extraction-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_DXF_DIR}"'/components/Universal Key Extraction.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_DXF_DIR}"'/components/Universal Key Extraction.mp'   -prohibit-legacy-codegen
   mpjret=$?
