...
# Deployed execution script for graph publish.ids compiled at 2016-10-25 09:28:38 using Co>Operating System version 3.2.6.4
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.distribution/main/users/akad/aicoe/pub_dxf}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=publish.ids

_AB_PROXY_DIR="$(pwd)/"publish.ids-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/publish.ids.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/publish.ids.mp'   -prohibit-legacy-codegen
   mpjret=$?
