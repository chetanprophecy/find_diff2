...
# Deployed execution script for graph determine_dates compiled at 2017-01-11 10:45:54 using Co>Operating System version 3.2.6.4
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.rxclaim/main/users/fjuang/aicoe/pub_dxf}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=determine_dates

# Host Setup Commands:
LD_LIBRARY_PATH=/opt/mapr/hiveodbc/lib/64:$LD_LIBRARY_PATH
LD_LIBRARY_PATH=/etl/opt/abinitio/abinitio-V3-2-6/misc/64/unixODBC-2.3.0/lib/libodbcinst.so.1:$LD_LIBRARY_PATH

MAPRHIVEINI=/home/fjuang/.mapr.hiveodbc.ini
MAPRINI=/home/fjuang/.mapr.hiveodbc.ini
export MAPRHIVEINI
export MAPRINI
export LD_PRELOAD
export LD_LIBRARY_PATH

#LD_PRELOAD=/etl/opt/abinitio/abinitio-V3-2-6/odbclocal/lib64/libodbcinst.so
LD_PRELOAD=/etl/opt/abinitio/abinitio-V3-2-6/misc/64/unixODBC-2.3.0/lib/libodbcinst.so.1
_AB_PROXY_DIR="$(pwd)/"determine_dates-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/determine_dates.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/determine_dates.mp'   -prohibit-legacy-codegen
   mpjret=$?
