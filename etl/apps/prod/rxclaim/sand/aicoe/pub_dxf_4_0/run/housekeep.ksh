...
# Deployed execution script for graph housekeep compiled at 2017-07-05 05:43:22 using Co>Operating System version 3.2.6.4
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.rxclaim/main/users/schakal1/aicoe/pub_dxf}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=housekeep

_AB_PROXY_DIR="$(pwd)/"housekeep-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/housekeep.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/housekeep.mp'   -prohibit-legacy-codegen
   mpjret=$?
