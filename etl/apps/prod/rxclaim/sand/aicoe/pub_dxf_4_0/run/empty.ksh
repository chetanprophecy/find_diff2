...
# Deployed execution script for graph empty compiled at 2016-04-27 07:12:15 using Co>Operating System version 3.2.4.1
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=empty

_AB_PROXY_DIR="$(pwd)/"empty-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_DXF_DIR}"'/mp/empty.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_DXF_DIR}"'/mp/empty.mp'   -prohibit-legacy-codegen
   mpjret=$?
