...
# Deployed execution script for graph ids_table_properties compiled at 2016-04-27 05:32:20 using Co>Operating System version 3.2.4.1
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=ids_table_properties

_AB_PROXY_DIR="$(pwd)/"ids_table_properties-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_DXF_DIR}"'/mp/ids_table_properties.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_DXF_DIR}"'/mp/ids_table_properties.mp'   -prohibit-legacy-codegen
   mpjret=$?
