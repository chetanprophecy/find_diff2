...
# Deployed execution script for graph generate_split_default_record compiled at 2017-05-10 09:58:47 using Co>Operating System version 3.2.6.4
# 
...
it.  Please define the environment variable PUB_DXF_DIR to be the project
...
# Project parameters:
export PUB_DXF_DIR;PUB_DXF_DIR=${PUB_DXF_DIR:-"$(cd ${__ab_script_dir}/..; pwd)"}
case "$_ab_uname" in
CYGWIN_* )
   PUB_DXF_DIR="$(cygpath -m "$PUB_DXF_DIR")"
esac
typeset _AB_SAVED_PROJECT_DIR
_AB_SAVED_PROJECT_DIR="${PUB_DXF_DIR}"
export AB_GRAPH_NAME;AB_GRAPH_NAME=generate_split_default_record

_AB_PROXY_DIR="$(pwd)/"generate_split_default_record-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_DXF_DIR}"'/mp/generate_split_default_record.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_DXF_DIR}"'/mp/generate_split_default_record.mp'   -prohibit-legacy-codegen
   mpjret=$?
