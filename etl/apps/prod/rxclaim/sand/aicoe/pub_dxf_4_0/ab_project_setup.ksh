...
if [ X"${1}" != X"" ]; then
   export PUB_DXF_DIR;   PUB_DXF_DIR="${1}"
   shift
fi
if [ X"${PUB_DXF_DIR:-}" = X"" ]; then
   print -r -- 'PUB_DXF_DIR is undefined.  Pass its value as the first argument to this script'
   export __AB_ERROR;   __AB_ERROR=1
fi
case $_ab_uname in
CYGWIN_* )
   PUB_DXF_DIR="${PUB_DXF_DIR:+$(cygpath -m "$PUB_DXF_DIR")}"
esac
if [ X"${__AB_ERROR}" != X1 ]; then
   _AB_CALLING_PROJECT=""
   if [ $# -gt 0 ]; then
      __AB_INVOKE_PROJECT "${PUB_DXF_DIR}"/.project.ksh "${PUB_DXF_DIR}" define start "$@"
   else
      __AB_INVOKE_PROJECT "${PUB_DXF_DIR}"/.project.ksh "${PUB_DXF_DIR}" define start
   fi
fi
...
function unset_project_env {
   _AB_CALLING_PROJECT=""
   if [ $# -gt 0 ]; then
      __AB_INVOKE_PROJECT "${PUB_DXF_DIR}"/.project.ksh "${PUB_DXF_DIR}" undefine start "$@"
   else
      __AB_INVOKE_PROJECT "${PUB_DXF_DIR}"/.project.ksh "${PUB_DXF_DIR}" undefine start
   fi
};
