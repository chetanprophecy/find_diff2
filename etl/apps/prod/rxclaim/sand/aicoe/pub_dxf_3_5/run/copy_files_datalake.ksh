...
# Deployed execution script for graph copy_files_datalake compiled at 2016-12-06 16:06:06 using Co>Operating System version 3.2.6.4
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.specialty/main/users/rgoel103/aicoe/pub_dxf}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=copy_files_datalake

_AB_PROXY_DIR="$(pwd)/"copy_files_datalake-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/copy_files_datalake.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/copy_files_datalake.mp'   -prohibit-legacy-codegen
   mpjret=$?
