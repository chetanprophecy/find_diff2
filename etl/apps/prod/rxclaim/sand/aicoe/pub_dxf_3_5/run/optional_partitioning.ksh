...
# Deployed execution script for graph optional_partitioning compiled at 2016-10-13 08:33:26 using Co>Operating System version 3.2.6.4
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=optional_partitioning

_AB_PROXY_DIR="$(pwd)/"optional_partitioning-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_DXF_DIR}"'/components/optional_partitioning.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_DXF_DIR}"'/components/optional_partitioning.mp'   -prohibit-legacy-codegen
   mpjret=$?
