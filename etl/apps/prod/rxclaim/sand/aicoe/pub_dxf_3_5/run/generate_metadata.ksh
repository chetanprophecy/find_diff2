...
# Deployed execution script for graph generate_metadata compiled at 2016-03-24 04:09:13 using Co>Operating System version 3.2.4.1
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.rxclaim/main/users/smishra/aicoe/pub_dxf}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=generate_metadata

_AB_PROXY_DIR="$(pwd)/"generate_metadata-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/generate_metadata.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/generate_metadata.mp'   -prohibit-legacy-codegen
   mpjret=$?
