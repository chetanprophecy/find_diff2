...
# Deployed execution script for graph serialize_input compiled at 2016-03-23 09:47:40 using Co>Operating System version 3.2.4.1
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=serialize_input

_AB_PROXY_DIR="$(pwd)/"serialize_input-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/serialize_input.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/serialize_input.mp'   -prohibit-legacy-codegen
   mpjret=$?
