...
# Deployed execution script for graph get.db_multi2 compiled at 2016-06-14 20:33:56 using Co>Operating System version 3.2.6.4
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.rebates/main/users/msing114/aicoe/pub_dxf}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=get.db_multi2

# Host Setup Commands:
. /etl/etc/environment
_AB_PROXY_DIR="$(pwd)/"get.db_multi2-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/get.db_multi2.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/get.db_multi2.mp'   -prohibit-legacy-codegen
   mpjret=$?
