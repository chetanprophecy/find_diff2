...
# Deployed execution script for graph add_audit_fields compiled at 2019-04-02 10:20:29 using Co>Operating System version 3.2.6.4
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.rxclaim/main/users/j17/aicoe/pub_datahub}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=add_audit_fields

_AB_PROXY_DIR="$(pwd)/"add_audit_fields-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/add_audit_fields.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/add_audit_fields.mp'   -prohibit-legacy-codegen
   mpjret=$?
