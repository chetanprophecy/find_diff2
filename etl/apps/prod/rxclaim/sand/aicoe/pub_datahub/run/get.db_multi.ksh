...
# 
export AB_HOME;AB_HOME=${AB_HOME:-/app01/abinitio/abinitio-V3-2-4}
...
# Deployed execution script for graph get.db_multi compiled at 2015-12-07 05:36:16 using Co>Operating System version 3.2.4.1
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/app01/abinitio/sand/nnagraj/aicoe/pub_datahub}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=get.db_multi

_AB_PROXY_DIR="$(pwd)/"get.db_multi-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/get.db_multi.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/get.db_multi.mp'   -prohibit-legacy-codegen
   mpjret=$?
