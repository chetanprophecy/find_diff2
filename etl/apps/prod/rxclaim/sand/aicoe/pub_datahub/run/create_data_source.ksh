...
# Deployed execution script for graph create_data_source compiled at 2016-07-08 03:55:38 using Co>Operating System version 3.2.4.1
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.datahub/main/users/akad/edw/datahub_dev/aicoe/pub_datahub}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=create_data_source

_AB_PROXY_DIR="$(pwd)/"create_data_source-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/create_data_source.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/create_data_source.mp'   -prohibit-legacy-codegen
   mpjret=$?
