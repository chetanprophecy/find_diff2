...
# Deployed execution script for graph read_remote_multi_icffs compiled at 2016-06-26 12:02:17 using Co>Operating System version 3.2.4.1
# 
...
it.  Please define the environment variable PUB_DATAHUB_DIR to be the project
...
# Project parameters:
export PUB_DATAHUB_DIR;PUB_DATAHUB_DIR=${PUB_DATAHUB_DIR:-"$(cd ${__ab_script_dir}/..; pwd)"}
case "$_ab_uname" in
CYGWIN_* )
   PUB_DATAHUB_DIR="$(cygpath -m "$PUB_DATAHUB_DIR")"
esac
typeset _AB_SAVED_PROJECT_DIR
_AB_SAVED_PROJECT_DIR="${PUB_DATAHUB_DIR}"
export AB_GRAPH_NAME;AB_GRAPH_NAME=read_remote_multi_icffs

_AB_PROXY_DIR="$(pwd)/"read_remote_multi_icffs-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_DATAHUB_DIR}"'/components/read_remote_multi_icffs.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_DATAHUB_DIR}"'/components/read_remote_multi_icffs.mp'   -prohibit-legacy-codegen
   mpjret=$?
