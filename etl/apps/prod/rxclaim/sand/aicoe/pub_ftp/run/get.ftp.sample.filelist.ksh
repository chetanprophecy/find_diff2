...
# Deployed execution script for graph "get.ftp.sample.filelist", compiled at Tuesday, June 18, 2013 12:20:35 using GDE version 3.1.5
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=get_ftp_sample_filelist

_AB_PROXY_DIR="$(pwd)"/get.ftp.sample.filelist-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/get.ftp.sample.filelist.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/get.ftp.sample.filelist.mp'   -prohibit-legacy-codegen
   mpjret=$?
