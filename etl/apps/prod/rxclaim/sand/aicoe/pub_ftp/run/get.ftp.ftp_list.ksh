...
# Deployed execution script for graph get.ftp.ftp_list compiled at 2017-08-23 11:19:22 using Co>Operating System version 3.2.6.4
# 
...
# Project parameters:
export PROJECT_DIR;PROJECT_DIR=${PROJECT_DIR:-/etl/devel/appconf.reg_dash/main/users/pjagtap3/aicoe/pub_ftp}
case "$_ab_uname" in
CYGWIN_* )
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=get.ftp.ftp_list

_AB_PROXY_DIR="$(pwd)/"get.ftp.ftp_list-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PROJECT_DIR}"'/mp/get.ftp.ftp_list.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PROJECT_DIR}"'/mp/get.ftp.ftp_list.mp'   -prohibit-legacy-codegen
   mpjret=$?
