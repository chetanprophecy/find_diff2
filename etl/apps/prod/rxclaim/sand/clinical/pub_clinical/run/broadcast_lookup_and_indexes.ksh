...
# Deployed execution script for graph broadcast_lookup_and_indexes compiled at 2016-11-22 20:29:16 using Co>Operating System version 3.2.6.4
# 
...
it.  Please define the environment variable PUB_CLINICAL_DIR to be the project
...
# Project parameters:
export PUB_CLINICAL_DIR;PUB_CLINICAL_DIR=${PUB_CLINICAL_DIR:-"$(cd ${__ab_script_dir}/..; pwd)"}
case "$_ab_uname" in
CYGWIN_* )
   PUB_CLINICAL_DIR="$(cygpath -m "$PUB_CLINICAL_DIR")"
esac
typeset _AB_SAVED_PROJECT_DIR
_AB_SAVED_PROJECT_DIR="${PUB_CLINICAL_DIR}"
export AB_GRAPH_NAME;AB_GRAPH_NAME=broadcast_lookup_and_indexes

_AB_PROXY_DIR="$(pwd)/"broadcast_lookup_and_indexes-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_CLINICAL_DIR}"'/components/broadcast_lookup_and_indexes.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_CLINICAL_DIR}"'/components/broadcast_lookup_and_indexes.mp'   -prohibit-legacy-codegen
   mpjret=$?
