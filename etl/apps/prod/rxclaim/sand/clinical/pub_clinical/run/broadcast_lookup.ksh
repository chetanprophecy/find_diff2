...
# Deployed execution script for graph broadcast_lookup compiled at 2016-11-22 20:28:33 using Co>Operating System version 3.2.6.4
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=broadcast_lookup

_AB_PROXY_DIR="$(pwd)/"broadcast_lookup-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_CLINICAL_DIR}"'/components/broadcast_lookup.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_CLINICAL_DIR}"'/components/broadcast_lookup.mp'   -prohibit-legacy-codegen
   mpjret=$?
