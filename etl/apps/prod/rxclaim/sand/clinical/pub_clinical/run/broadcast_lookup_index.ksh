...
# Deployed execution script for graph broadcast_lookup_index compiled at 2016-11-22 20:29:07 using Co>Operating System version 3.2.6.4
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=broadcast_lookup_index

_AB_PROXY_DIR="$(pwd)/"broadcast_lookup_index-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_CLINICAL_DIR}"'/components/broadcast_lookup_index.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_CLINICAL_DIR}"'/components/broadcast_lookup_index.mp'   -prohibit-legacy-codegen
   mpjret=$?
