...
# Deployed execution script for graph load_transactions compiled at 2016-12-23 08:54:18 using Co>Operating System version 3.2.6.4
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=load_transactions

_AB_PROXY_DIR="$(pwd)/"load_transactions-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_CLINICAL_DIR}"'/components/load_transactions.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_CLINICAL_DIR}"'/components/load_transactions.mp'   -prohibit-legacy-codegen
   mpjret=$?
