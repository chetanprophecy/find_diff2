...
# Deployed execution script for graph send_mail compiled at 2017-09-14 09:23:04 using Co>Operating System version 3.2.6.4
# 
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=send_mail

_AB_PROXY_DIR="$(pwd)/"send_mail-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
if [ $# -gt 0 ]; then
   air sandbox run "${PUB_CLINICAL_DIR}"'/mp/send_mail.mp' "$@"  -prohibit-legacy-codegen
   mpjret=$?
else
   air sandbox run "${PUB_CLINICAL_DIR}"'/mp/send_mail.mp'   -prohibit-legacy-codegen
   mpjret=$?
